...
class KnowledgeMarketAgent(AgentBase):
    ...
    def __init__(self, name: str, USD: float, OCEAN: float,
        ...
        self._receiving_agents: dict = fee_receiving_agents
    ...
    def _ToDistribute(self, state):
        ...
        if received > 0:
            ...
            for researcher in state.researchers.keys():
                if state.getAgent(researcher).last_tick_spent == state.tick or state.getAgent(researcher).last_tick_spent == (state.tick - 1):
